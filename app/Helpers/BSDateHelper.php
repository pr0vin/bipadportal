...
class BSDateHelper
{
     ...
     * @return string
     */
    private static function _get_nepali_month($m)
    {
        $n_month = FALSE;
        switch ($m) {
            case 1:
    ...
    public static function eng_to_nep($yy, $mm, $dd)
    {
        ...
        if ($chk !== TRUE) {
            ...
            $_nep_date['nmonth'] = self::_get_nepali_month($m);
            $_nep_date['num_day'] = self::convert_to_nepali_number($numDay);

            //english
            $_nep_date['year_en'] = $y;
            $_nep_date['month_en'] = $m;
            $_nep_date['date_en'] = $total_nDays;
            $_nep_date['day_en'] = self::_get_day_of_week($day);
            $_nep_date['nmonth_en'] = $m;
            return $_nep_date;
        }
    }
    ...
    public static function AdToBsReadable($delimiter, $date)
    {
        $date_en = "";
        if (!empty($date)) {
            $np_date = explode($delimiter, $date);
            $date_np = self::eng_to_nep($np_date[0], $np_date[1], $np_date[2]);
            return $date_np['day'] . ', ' . $date_np['nmonth'] . ' ' . $date_np['date'] . ', ' . $date_np['year'];
        }
        return $date_en;
    }
    ...
    public static function AdToBsEN($delimiter, $date)
    {
        ...
        if (!empty($date)) {
            ...
            return $date_np['year_en'] . '/' . str_pad($date_np['month_en'], 2, "0", STR_PAD_LEFT) .
                '/' . str_pad($date_np['date_en'], 2, "0", STR_PAD_LEFT);
