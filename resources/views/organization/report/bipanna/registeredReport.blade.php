@extends('layouts.app')

@section('content')
    <div class="container-fluid">
        @include('partials.bipanna-tab')
        <div class="card z-depth-0 printableCard">
            <div class="card-body">

                <div class="d-flex justify-content-end">
                    <button type="button" class="btn btn-primary" id="report_print"><i class="fa fa-print"
                            aria-hidden="true"></i>
                        Print</button>
                    {{-- @php
                        $fullUrl = url()->full() . '&excel=1';
                    @endphp --}}
                    {{-- <a href="{{ $fullUrl }}" class="mr-1 btn btn-primary pt-2"><i class="fa fa-file-excel"
                            aria-hidden="true"></i>
                        Excel</a> --}}
                </div>

                <div class="my-3"></div>
                <div id="printReport" class="m-0 p-0">

                    <div clas="">
                        @if (Session()->get('municipality_id'))
                            <h3 class="text-center kalimati-font">
                                {{ App\Organization::where('address_id', Session()->get('municipality_id'))->first()->address->municipality }}
                                स्वास्थ्य शाखा</h3>
                            <h5 class="font-weight-bold text-center">(सिफारिस भएका)</h5>
                        @endif
                        <p class="text-center text-dark kalimati-font">
                            {{-- {{ $this->message }} --}}
                        </p>
                    </div>
                    <div class="py-4">
                        <table class="table table-bordered table-sm table_print kalimati-font">
                            <thead class="font-noto">
                                <tr class="text-center" style="font-size: 11px">
                                    <th>क्र.स</th>
                                    <th>लाभग्राहिको नाम थर</th>
                                    <th>लिङ्ग</th>
                                    <th>उमेर(बर्ष)</th>
                                    <th>ठेगाना</th>
                                    <th>रोगको किसिम</th>
                                    <th>सिफारिस मिति</th>
                                    <th>रा.प.प.नं../ना.प्र.प.नं./जन्म दर्ता प्र.प.नं.</th>
                                    <th>मोबाइल नम्बर</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($patients as $patient)
                                    <tr class="text-center">
                                        <td class="kalimati-font">{{ $loop->iteration }}</td>
                                        <td>{{ $patient->name }}</td>
                                        <td>
                                            @if ($patient->gender == 'Male' || $patient->gender == 'male')
                                                पु
                                            @elseif($patient->gender == 'Female' || $patient->gender == 'female')
                                                म
                                            @else
                                                अ
                                            @endif
                                        </td>
                                        <td class="kalimati-font">{{ $patient->age }} बर्ष</td>
                                        <td class="kalimati-font">{{ $patient->address->municipality }} - {{$patient->ward_number}}</td>
                                        <td>{{ $patient->disease->name }}</td>
                                        <td class="kalimati-font">{{ $patient->registered_date }}</td>
                                        <td class="kalimati-font">{{ $patient->citizenship_number }}</td>
                                        <td class="kalimati-font">{{ $patient->mobile_number }}</td>
                                    </tr>
                                @endforeach
                                {{-- @foreach ($patientLists as $patientList)
                                    <tr class="text-center">
                                        <td>{{ $loop->iteration }}</td>
                                        <td>
                                            @php
                                                print_r($patientList['hospital']);
                                            @endphp
                                        </td>
                                        <td>
                                            @php
                                                print_r($patientList['female']);
                                            @endphp
                                        </td>
                                        <td>
                                            @php
                                                print_r($patientList['male']);
                                            @endphp
                                        </td>
                                        <td>
                                            @php
                                                print_r($patientList['other']);
                                            @endphp
                                        </td>
                                        <td>
                                            @php
                                                print_r($patientList['total']);
                                            @endphp
                                        </td>
                                    </tr>
                                @endforeach --}}
                            </tbody>

                        </table>
                    </div>
                </div>
            </div>
        </div>
        <div class="card mt-3 m-0 p-0">

        </div>
        @push('scripts')
            <script>
                $(document).ready(function() {

                    $("#report_print").click(() => {


                        window.print();


                    })
                })
            </script>
        @endpush

    </div>
@endsection
