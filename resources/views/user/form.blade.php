                            ...
                            <form action="{{ isset($user->id) ? route('user.update', $user) : route('user.store') }}"
                                ...
                                <div class="form-group">
                                    ...
                                    @foreach ($roles as $role)
                                        ...
                                        <div class="custom-control custom-checkbox">
                                            ...
                                            <label class="pl-4 custom-control-label"
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="municipality_id" id="" class="custom-select">
                                            @foreach ($municipalities as $municipality)
                                                <option value="{{ $municipality->address->id }}"
                                                    {{ municipalityId() == $municipality->address->id ? 'selected' : '' }}>
                                                    {{ $municipality->address->municipality }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                @else
                                ...
                                <div class="form-group">
                                    <div class="custom-control m-0 p-0 custom-checkbox">
                                        <input type="checkbox" class="custom-control-input" name="is_ward_login"
                                            id="ward-toggler" @if (($user->id && isset($user->ward_id)) || !isset($user->id)) checked @endif>
                                        <label class="custom-control-label pl-4" for="ward-toggler">Ward Login</label>
                                        <small class="form-text text-muted">
                                            Check this field for ward employees and make sure to select the ward from
                                            dropdown below.
                                        </small>
                                    </div>
                                </div>
                                    ...
                                    <select name="ward_id" class="custom-select">
                                        ...
                                        @foreach ($wards as $ward)
                                            <option value="{{ $ward->id }}"
                                                @if (old('ward_id', $user->ward_id) == $ward->id) selected @endif>{{ $ward->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
